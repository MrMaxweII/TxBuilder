...
/**	Konstruktor der das JFrame Fenster öffnet. (Besser in einem neuem Thread öffnen)
	...
	@param joTx	 Transaktion als JSONObject. Bei Unsignierten Tx ist dies z.B. der Fall.
	@param x	 X-Position des JFrames
	@param y	 Y-Position des JFrames  **/ 
//	public TxPrinter(byte[] magic, JSONObject joTx, int x, int y) throws JSONException 
//	{
//		// Hier wird mit "this" der nächste Konstruktor aufgerufen.
//		this(magic,new Transaktion(Convert.hexStringToByteArray(joTx.getString("hex")),0),x,y);
//		if(joTx.getBoolean("signed")) 	signed = "true";                 <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Hier weitermachen: Anweiseung werden zu spät ausgeführt!
//		else							signed = "false";
//	}
